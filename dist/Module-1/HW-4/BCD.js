...
class BCD extends Adder {
    ...
    get isNegative() {
        return (this.numbers[0] & (0, helpers_js_1.getMask)(6)) >>> 24 !== 0;
    }
    get lengthOfLastNumber() {
        return ((0, helpers_js_1.getMask)(7) & this.numbers.at(-1)) >>> 28;
    }
    set lengthOfLastNumber(num) {
        this.bcd |=
            (0, helpers_js_1.getNumberLength)(num) <<
    ...
    getNumberPos(ind) {
        const numberLength = (0, helpers_js_1.getNumberLength)(this.number);
    ...
    getPow(num, res) {
        const numberLength = (0, helpers_js_1.getNumberLength)(num);
    ...
    toBcd() {
        ...
        while (number) {
            this.bcd = 0;
            let pos = 0;
            let [chunk, nextChunk] = (0, helpers_js_1.divmod)(Math.abs(number), 10 ** this.getPow(number, result));
    ...
    valueOf() {
        console.log("NUMBERS: ", this.numbers.map((n) => (0, helpers_js_1.binary)(n, 32)));
        return this.numbers.map(BigInt).reduce((combinedNumber, number, ind) => {
            ...
            if (ind === 1) {
                combinedNumber = (0, helpers_js_1.trim)(combinedNumber, 5);
            }
            number = (0, helpers_js_1.trim)(number, 6);
    ...
    get(ind) {
        ...
        const union = number & (0, helpers_js_1.getMask)(pos);
        return this.getResolvedBcdDigit(union >>> (pos * this._bcdDigitSize));
    }
    resolvedBcdSum(isAddendNegative, areAddendsEqual) {
        return (a, b, carry = 0) => {
            const digits = [
                { digit: a, isNegative: this.isNegative },
                { digit: b, isNegative: isAddendNegative },
            ];
            const [rDigit1, rDigit2] = digits.map(({ digit, isNegative }) => this.getResolvedBcdDigit(digit % 10, isNegative));
    ...
    add(addend) {
        ...
        let pos = 0;
        let previousCarry = 0;
        let isResultNegative = (0, helpers_js_1.getIsResultNegative)(number, addend);
        const isAddendNegative = addend < 0;
        const areAddendsEqual = Math.abs(number) === Math.abs(addend);
        const end = Math.max((0, helpers_js_1.getNumberLength)(number), (0, helpers_js_1.getNumberLength)(addend));
        const resolvedBcdSum = this.resolvedBcdSum(isAddendNegative, areAddendsEqual);
        if (isResultNegative) {
        ...
        while (pos < end) {
            ...
            previousCarry = carry;
            res |= sum << shift;
            number = (0, helpers_js_1.getNextNumber)(number);
            addend = (0, helpers_js_1.getNextNumber)(addend);
            pos++;
        }
        if (((this.isNegative || isAddendNegative) &&
            !isResultNegative &&
            !areAddendsEqual) ||
            (this.isNegative && isAddendNegative)) {
            res = this.adder(res, 1);
        }
        console.log("sum: ", (0, helpers_js_1.binary)(res, 32));
        return res;
    }
    subtract(num) {
        const number = ~num + 1;
